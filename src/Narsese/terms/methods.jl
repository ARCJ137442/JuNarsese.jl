...
begin "检查合法性（API接口，用于后续NAL识别）"
    ...
    整体类型依赖逻辑：
        ...
        外部非解释　　　　 ⇐　外部可解释（最慢）
    """
    check_valid(::Term) = true # 默认为真（最基础地只需修改这个）
    check_valid_external(t::Term) = check_valid(t) # 默认重定向
    check_valid_explainable(t::Term) = check_valid(t) ? t : error("非法词项！") # 默认重构「非解释合法性检查」逻辑
    check_valid_external_explainable(t::Term) = check_valid_explainable(t) # 默认重定向
