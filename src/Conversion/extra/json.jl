...
begin "基础方法集"
    ...
    function preprocess(::Type{T}, val::Any) where T
        val # 数字/字符串
    end
    
    "预处理：表达式⇒字典"
    preprocess(::Type{Dict}, ast::Expr)::Dict = Dict(
        string(ast.head) => preprocess.(Dict, ast.args) # 批量处理
    )

    "预处理：表达式⇒数组（向量）"
    preprocess(::Type{Vector}, ast::Expr)::Vector = [
        string(ast.head), # 头
        preprocess.(Vector, ast.args)... # 批量处理并展开
    ]

    "默认方法：用于递归处理参数（无需类型判断，只需使用多分派）"
    preparse(::Type, v::Any) = v # 数字/字符串

    "预解析：字典⇒表达式"
    preparse(::Type{Dict}, d::Dict)::Expr = begin
        pair::Pair = collect(d)[1]
        return Expr(
            Symbol(pair.first), # 只取第一个键当类名
            preparse.(Dict, pair.second)..., # 只取第一个值当做参数集
        )
    end

    "预解析：数组（向量）⇒表达式"
    preparse(::Type{Vector}, v::Vector)::Expr = Expr(
        Symbol(v[1]), # 取第一个当类名
        preparse.(Vector, v[2:end])..., # 其后当做参数
    )
end
